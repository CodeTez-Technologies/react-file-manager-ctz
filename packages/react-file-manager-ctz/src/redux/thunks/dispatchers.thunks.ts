...
import { ExplorerDispatch, ExplorerThunk } from '../../types/redux.types';
...
export const thunkDispatchFileAction =
  (data: FileActionData<FileAction>): ExplorerThunk =>
    (_dispatch, getState) => {
 ...
 * actions. When action is requested, Explorer "prepares" the action data by extracting it
 * from Redux state. Once action data is ready, Explorer executes some side effect and/or
 * dispatches the action to the external action handler.
 */
export const thunkRequestFileAction =
  <Action extends FileAction>(action: Action, payload: Action['__payloadType']): ExplorerThunk =>
    (dispatch, getState) => {
